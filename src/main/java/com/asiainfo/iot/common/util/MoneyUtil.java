...
public class MoneyUtil {
	...
	public static String CNValueOf(String moneyValue) {  
		...
		if (dot_pos == -1) {  
			...
			+ "0000".substring(0, 4);//也加两个0，便于后面统一处理  
		}  
		...
		int fv4 = Integer.parseInt(fraction_value.substring(3,4));
		if(fv1+fv2==0){  
			...
			cn_currency.append(CN_Digits[fv4]).append("毫");  
		}  
		String resultCurrency = cn_currency.toString();
		if (resultCurrency.contains("零毫")) {
			resultCurrency=resultCurrency.replace("零毫", "");
			if (resultCurrency.contains("零厘")) {
				resultCurrency=resultCurrency.replace("零厘", "");
				if (resultCurrency.contains("零分")) {
					resultCurrency=resultCurrency.replace("零分", "");
					if (resultCurrency.contains("零角")) {
						resultCurrency=resultCurrency.replace("零角", "");
					}
				}
			}
		}
		if(resultCurrency.startsWith("圆")) resultCurrency = "零" + resultCurrency;
